...
// function should produce 2 things
// the number of email messages in the string
...
let uselessRegex = /\bFrom: ?\b|\bTo: ?\b|\bSubject: ?\b|\bDate: ?\b|\n?/g
const mailCount = rawString => {
  rawString = rawString.replaceAll(uselessRegex, '');
  let messages = rawString.split('##||##');
  let processedMessages = messages.map((message) => {
    ...
    return {
      ...
      date: messageData[2],
      recipient: messageData[3],
      body: messageData[4],
    }
  });
  let dates = processDates(processedMessages);
  dates.sort((a, b) => {
    if (a > b) {
      return 1;
    } else if (a < b) {
  ...
  console.log(`Count of Email ${processedMessages.length}`);
  console.log(`Date Range: ${dates[0].toDateString()} - ${dates[dates.length - 1].toDateString()}`)
}

const processDates = messageData => {
  return messageData.map(message => new Date(message.date));
};
